...
def create_coarse_mask(fine_mask, erosion_kernel_size=5, dilation_kernel_size=7):
    """
    Tạo coarse mask từ fine mask bằng cách sử dụng morphological operations
    """
    if not np.any(fine_mask):
        return np.zeros_like(fine_mask, dtype=np.uint8)
    
    # Erosion để làm nhỏ mask
    kernel_erosion = np.ones((erosion_kernel_size, erosion_kernel_size), np.uint8)
    eroded = cv2.erode(fine_mask.astype(np.uint8), kernel_erosion, iterations=1)
    
    # Dilation để làm to mask (tạo coarse boundary)
    kernel_dilation = np.ones((dilation_kernel_size, dilation_kernel_size), np.uint8)
    coarse_mask = cv2.dilate(eroded, kernel_dilation, iterations=1)
...
def create_cocoa_format(coco_json: str, output_json: str, depth_dir: str):
    ...
    for img in data['images']:
        ...
        for i in range(N):
            ...
            # Tạo coarse masks từ fine masks
            visible_binary = (visible_mask > 0).astype(np.uint8)
        
            coarse_visible_mask = create_coarse_mask(visible_binary, erosion_kernel_size=3, dilation_kernel_size=5) if np.any(visible_binary) else np.zeros_like(visible_binary)
            
            # Tạo polygons cho segmentation (fine masks)
            ...
            # Tạo polygons cho coarse masks
            coarse_visible_polygon = mask_to_polygon(coarse_visible_mask) if np.any(coarse_visible_mask) else []
            ...
            annotation = {
                ...
                "amodal_mask": [amodal_polygon],  # Fine-grained amodal mask
                "bbox": amodal_bbox,              
                "visible_mask": [visible_polygon] if len(visible_polygon) > 0 else [], # Fine-grained visible mask
                "coarse_visible_mask": [coarse_visible_polygon] if len(coarse_visible_polygon) > 0 else [], # Coarse visible mask
                "iscrowd": 0,
    ...
    cocoa_data = {
        ...
        "info": {
            "description": "COCOA format dataset for amodal segmentation with coarse masks",
            "version": "1.0",
            "year": 2024
        }
    }
    ...
    print(f"✅ Đã tạo file COCOA format với coarse masks: {output_json}")
    print(f"📊 Tổng số ảnh: {len(images)}")
    print(f"📊 Tổng số annotations: {len(annotations)}")
    print(f"💡 Mỗi annotation bao gồm: fine + coarse masks cho cả amodal & visible")
